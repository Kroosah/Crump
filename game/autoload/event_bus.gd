...
## (taak 009) en het audiosysteem (taak 005) abonneren zich hierop.
...
## De speler is waargenomen door de dreiging (taak 009 zendt dit).
...
## Een leesbaar document is geopend (taak 003/007). De lees-UI (een
## verwijderbaar systeem) toont `title` en `text` letterlijk; lege titel =
## geen titelregel. Het briefje zelf heeft GameState al bijgewerkt en
## valideerde de data — er komt nooit een lege id of lege tekst op de bus.
## Lezen is stil — dit signaal gaat bewust niet vergezeld van noise_made
## (pijler 1). Eenmalig gecorrigeerd van 2 naar 3 argumenten vóór de
## eerste productieconsumer (D-029); vanaf nu geconsumeerd contract
## (D-022-regime).
...
## geslaagde statewijziging. CRUMP's zicht (taak 009) abonneert zich; zonder
